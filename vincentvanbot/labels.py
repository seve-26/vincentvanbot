...
if __name__ == '__main__':
    ...
    df_total = get_data_locally(100_000)
    
    import numpy as np
    df_list = []
    done = 96 # last number shown
    for i, df in enumerate(np.array_split(df_total,100)):
        if i < done - 1:
            continue
        print(f"\nWorking on slice {i+1}")
        labels_df = get_labels_df(df,100_000, source='local', manual=False)
        filename = join(os.path.dirname(__file__),'..','raw_data','labels',f'labels_{i+1}.pkl') 
        labels_df.to_pickle(filename)
